...
class TestRadiativeRecombination(unittest.TestCase):
    ...
    def test_radiative_recombination_zero(self):
        ...
        p = np.array([0, 0, 0])
        R_rad = radiative_recombination(n, p, B=1e-10, temperature=300)
        self.assertTrue(np.all(R_rad == 0), "Radiative recombination rate should be zero when carrier concentrations are zero")
    
    def test_radiative_recombination_scalar(self):
        n = 1e17
        p = 1e17
        R_rad = radiative_recombination(n, p, B=1e-10, temperature=300)
        self.assertEqual(R_rad, 1e-10 * (n * p - 1.5e10**2), "Incorrect radiative recombination rate for scalar inputs")
